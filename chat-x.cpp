...
void frnd::chat_on(string uname,string reciepient,char *message)
{
    ...
    strcpy(sms[reciepient].mess[sms[reciepient].no_msg],cuname);
    strcat(sms[reciepient].mess[sms[reciepient].no_msg],":  ");
    strcat(sms[reciepient].mess[sms[reciepient].no_msg],message);
    sms[reciepient].no_msg++;
}
...
void frnd::inbox(string uname)
{
    ...
    for(int i=0;i<=sms[uname].no_msg;i++)
    {
